...
function ChatRoom() {
  ...
  useEffect(() => {
    axios
      .post("http://localhost:5000/student/getRequestedAlumni", {
        email: localStorage.getItem("email"),
      })
      .then(async (output) => {
        setAlumni(output.data);
  ...
  if (data[chatIndex]["conversations"].length === 0) {
    renderedChat = <p className="text-left mt-2">No chats</p>;
  } else {
    renderedChat = data[chatIndex]["conversations"].map((value) => {
        if (value[1] === "alumni") {
  ...
  var sideChatChoice = alumni.map((value, index) => {
    return (
      <button
    ...
    <div className="w-full overflow-x-hidden">
      ...
      <div className="w-full flex flex-col items-center">
        <div className="w-[80%] flex h-[80vh]">
          <div className="w-[30%] text-white bg-black flex flex-col items-center overflow-y-scroll scroll-smooth">
            {sideChatChoice}
          </div>
          <div className="w-[70%] text-black bg-white p-4 scroll-smooth h-full overflow-y-scroll">
            {renderedChat}
          </div>
        </div>
        <form
          onClick={(e) => {
            ...
            console.log(message);
            axios
              .put("http://localhost:5000/student/addChat", {
                email: alumni[chatIndex],
